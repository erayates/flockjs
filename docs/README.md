# FlockJS Documentation

Audience: users and contributors.

This directory is the canonical Markdown documentation hub for FlockJS.

## Current State

FlockJS is under active development. API contracts and examples here define the intended direction, and some functionality is still planned.

## Getting Started

- [Installation](getting-started/installation.md)
- [Quickstart](getting-started/quickstart.md)
- [Rooms and transports](getting-started/rooms-and-transports.md)

## Reference

- [Reference index](reference/README.md)
- [Core API](reference/core-api.md)
- [Presence engine](reference/engines-presence.md)
- [Cursor engine](reference/engines-cursors.md)
- [State, awareness, events](reference/engines-state-awareness-events.md)
- [React adapter](reference/adapters-react.md)
- [Vue adapter](reference/adapters-vue.md)
- [Svelte adapter](reference/adapters-svelte.md)
- [Advanced features](reference/advanced.md)
- [UI components](reference/ui-components.md)
- [Devtools and debugging](reference/devtools-debugging.md)
- [Performance guide](reference/performance.md)
- [Type reference](reference/types.md)

## Recipes

- [Collaborative editor](recipes/collaborative-editor.md)
- [Multiplayer canvas](recipes/multiplayer-canvas.md)
- [Live voting](recipes/live-voting.md)
- [Presence-aware navigation](recipes/presence-aware-navigation.md)

## Project Docs

- [Repository structure](project/repository-structure.md)
- [Development setup](project/development-setup.md)
- [Code conventions](project/code-conventions.md)
- [WebRTC validation](project/webrtc-validation.md)
- [Execution plan](project/execution-plan.md)
- [Labeling and triage](project/labeling-and-triage.md)
- [Release process](project/release-process.md)
- [Documentation style guide](STYLE_GUIDE.md)

## Related Docs

- [Root README](../README.md)
- [Contributing](../CONTRIBUTING.md)
- [Roadmap](../ROADMAP.md)
